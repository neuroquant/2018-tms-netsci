...
from nilearn.signal import butterworth, clean
...
from nilearn._utils.niimg import load_niimg, _safe_get_data
...
def temporal_derivatives(order, variables, data):
    ...
    data_deriv = pandas.DataFrame(columns=variables_deriv,
...
def exponential_terms(order, variables, data):
    ...
    data_exp = pandas.DataFrame(columns=variables_exp,
...
def denoise(img_file, tsv_file, out_path, col_names=False, hp_filter=False, lp_filter=False, out_figure_path=False):
    ...
    motion = df[motion_names]
    ## square movement variables and add (Friston 12P)
    ## Edit: Removing below and using niworkflow functions instead. 
    # motion_squared=motion**2
    # motion_squared.columns=['%s**2'%i for i in motion_squared.columns]
    # df = pandas.concat([df, motion_squared], axis=1)
    ## Create Friston 24P
    motion_dd_names,motion_dd = temporal_derivatives(
                                [0,1],
                                motion_names,
                                motion)
    motion_ddsq_names,motion_ddsq = exponential_terms(
                                    [2],
                                    motion_dd_names,
                                    motion_dd)
    motion_dd = motion_dd[motion_dd_names[6:12]]
    motion_ddsq = pandas.concat([motion_dd, motion_ddsq], axis=1)
    df = pandas.concat([df, motion_ddsq], axis=1)
    ...
    if np.sum(fd_idx) > 0:
        FD_name = np.ravel(poss_names)[np.where(fd_idx)[0]][0]
    ...
    for idx, img_curr in enumerate(imgs):
        ...
        fig = plot_carpet(img_curr, mask_img=nifti_masker.mask_img_, figure=fig, axes=ax_curr)
    ...
    TEMPLATE_FILE = pjoin(os.getcwd(), "denoiser", "report_template.html")
